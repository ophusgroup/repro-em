...
The raw data is the measurement. Everything else on this site is about protecting the path from that measurement to a published claim, so it is worth being precise about what we mean by raw, and what has to be stored alongside it.
...
Every step past that point is a choice: binning, summing, gain correction, drift correction, cropping, and conversion to 8 or 16 bit. Each is usually reasonable and each is irreversible. Published data should preserve whatever the instrument permitted, and state plainly where the raw record actually begins.
...
**Honesty about what was kept.** If the full raw dataset genuinely cannot be published, publish the reduced product, state exactly what reduction was applied, and preserve the raw with an identifier and a stated retention period. Size is a real constraint. It is not a reason to publish nothing.
