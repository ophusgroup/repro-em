...
def main():
    ...
    rename = [("entry.client-PCJPW7TK", f"entry.client-{CACHE_TAG}"),
              ("manifest-C732C875", f"manifest-{CACHE_TAG}")]
    pub = os.path.join(THEME, "public", "build")
    if not os.path.exists(os.path.join(pub, f"entry.client-{CACHE_TAG}.js")):
        import shutil
        ...
        for path in [os.path.join(THEME, "build", "index.js")] + [
            os.path.join(pub, f"manifest-{CACHE_TAG}.js")
        ]:
            with open(path) as f:
                s = f.read()
            for old, new in rename:
